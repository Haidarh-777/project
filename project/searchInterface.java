...
public class searchInterface extends JFrame implements ActionListener {
    ...
    JLabel nodeInfoLabel; // لعرض معلومات العقدة إن وجدت
    ...
    searchInterface() {
        ...
        label.add(IDLabel);
        label.add(textLabel);
        label.add(nodeInfoLabel); // أضف label للمعلومات
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == submitButton) {
            ...
            if (existed) {
                ...
                // طباعة معلومات العقدة في الـ JLabel الجديد
                nodeInfoLabel.setText(
                        "<html>ID: " + node.id + "<br>" +
                                "Name: " + node.name + "<br>" +
                                "Price: " + node.price + "<br>" +
                                "Quantity: " + node.quantity + "<br>" +
                                "Priority: " + node.priority + "</html>");

            } else {
                JOptionPane.showMessageDialog(null, "The shipping parcel is not existed ",
                        " ", JOptionPane.OK_OPTION);

                // في حال لم يتم العثور، امسح المعلومات السابقة
                nodeInfoLabel.setText("");
            }
        }
